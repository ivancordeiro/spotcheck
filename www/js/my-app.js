...
if (document.location.hostname == "localhost") {
    ...
    */
    ...
    myApp.onPageInit('login', function(page) {
        ...
        escreverArquivo("sincronia.json", "", 0);
        escreverArquivo("spotcheck.json", "", 0);
        escreverArquivo("resposta.json", "", 0);
        escreverArquivo("query.json", "", 0); //Queries que serão executadas assim que houver conexão

        escreverArquivo("obra.json", "",0);
        escreverArquivo("cliente.json", "", apagar_jsons);
        escreverArquivo("gerencia.json", "", apagar_jsons);
        escreverArquivo("responsavel.json", "", apagar_jsons);
        escreverArquivo("setor.json", "", apagar_jsons);
        escreverArquivo("frente.json", "", apagar_jsons);
        escreverArquivo("localizacao.json", "", apagar_jsons);
        escreverArquivo("atividade.json", "", apagar_jsons);
        escreverArquivo("tema_em_uso.json", "", apagar_jsons);
        escreverArquivo("pergunta_em_uso.json", "", apagar_jsons);
        escreverArquivo("programacao.json", "", apagar_jsons);
        escreverArquivo("programacao_temas.json", "", apagar_jsons);
        escreverArquivo("terceiro.json", "", apagar_jsons);
        escreverArquivo("usuario.json", "", apagar_jsons);
    ...
    */
    ...
    myApp.onPageInit('programacao-atual', function(page) {
        ...
        getManyById_2(obra_id, "id_obra", aplicador_id, "aplicador", "programacao.json", function(itens) {
            ...
            for (var i = 0; i < total; i++) {
                ...
                if(contador > 0 || itens[i].completado == 1) {
                ...
                if(itens[i].completado == 0) { contador++; }
            ...
            if (programacao_id == 0) {
                ...
                getManyById(programacao_id, "id_programacao", "programacao_temas.json", function(itens) {
                    ...
                    for (var i = 0; i < total; i++) {
                        ...
                        html += '<li>';
                        html += '    <a data-completado="'+itens[i].completado+'" data-qtd="'+itens[i].qtd+'" data-tema="'+itens[i].tema_id+'" data-programacao="'+programacao_id+'" data-id="'+itens[i].id+'" data-nome="'+itens[i].nome+'" class="option-subprogramacao item-link item-content">';
    ...
    myApp.onPageInit('spotcheck-inicio', function(page) {
        ...
        if (typeof spotcheck_id == "undefined" || spotcheck_id == null) {
            ...
            getManyById(obra_id, "id_obra", "gerencia.json", function(gerencia) {
                ...
                var gerencia_html = "<option selected>Selecione</option>";
                for(var i = 0; i < total_gerencia; i++) {
                    gerencia_html += '<option value="'+gerencia[i].id+'">'+gerencia[i].nome+'</option>';
                }
    ...
    myApp.onPageInit('registra-rd', function(page) {
        ...
        $(".btn-proxima-pergunta").off().click(function() {
            ...
            if ($("input[name=criticidade]:checked").length < 1 || $("#foto1").val() == "" || $("#foto2").val() == "" || $("#foto3").val() == "" || $("#relato").val() == "" || $("#acoes_tomadas").val() == "" || recomen) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (data != "erro") {
                                    ...
                                    myApp.alert("Não foi possível realizar o envio do Spotcheck. Não foi identificada uma conexão com a internet.", "Sem conexão à internet!");
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (data != "erro") {
                                        ...
                                        myApp.alert("Não foi possível realizar o envio do Spotcheck. Por favor, tente novamente mais tarde.", "Erro ao enviar!");
                                ...
                                error: function(x, status) {
                                    myApp.alert("Não foi possível realizar o envio do Spotcheck. Não foi identificada uma conexão com a internet.", "Erro ao enviar!");
                                    $("#enviando-spotcheck").append('<div class="toolbar btn-iniciar btn-voltar-spotchecks"><a href="#" class="button button-fill button-big button-raised">Voltar para Spotchecks</a></div>');
                                    $("#enviando-spotcheck .btn-voltar-spotchecks").off().click(function() {
                                        mainView.router.loadPage({
                                            url: 'selecionar-spotcheck.html',
